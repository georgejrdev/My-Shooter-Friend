...
function getNamePlayers(): string[]{
    let namePlayerOne = (document.getElementById("player-name-one") as HTMLInputElement).value
    let namePlayerTwo = "Computer"

    if (GAME_STATUS.mode === "1v1") namePlayerTwo = (document.getElementById("player-name-two") as HTMLInputElement).value

    if (!namePlayerOne || !namePlayerTwo || namePlayerOne.trim() === "" || namePlayerTwo.trim() === "") return ["Lula","Bolsonaro"]

    return [namePlayerOne, namePlayerTwo]
}

function createPlayers(name: string): Player{
    return {
        name: name,
        life: 100,
        maxLife: 100,
...
export function checkGameOver(): void{
    ...
    if (playerOne.life <= 0) winner = playerTwo.name
    if (playerTwo.life <= 0) winner = playerOne.name
