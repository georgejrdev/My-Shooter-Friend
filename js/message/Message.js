...
export function setMessage() {
    if (MESSAGE.action === "attack")
        MESSAGE.message += `${MESSAGE.player} <span class='emphasis-gold'>attacks</span> ${MESSAGE.target}`;
    if (MESSAGE.action === "defense")
        MESSAGE.message += `${MESSAGE.player} <span class='emphasis-gold'>defended himself</span>`;
    if (MESSAGE.action === "effect")
        MESSAGE.message += `<span class='emphasis-gold'>Effect</span> for ${MESSAGE.player}: ${MESSAGE.effect}`;
    if (MESSAGE.typeAttack != "")
        MESSAGE.message += ` | ${MESSAGE.typeAttack} <span class='emphasis-light-orange'>attack</span>`;
    if (MESSAGE.extraDamage != 0)
        MESSAGE.message += ` | <span class='emphasis-light-red'>Extra damage:</span> ${MESSAGE.extraDamage}`;
