...
function handleKeyPress(key, pressedByComputer) {
    if (!validationKeyPress(key, pressedByComputer))
        return;
...
function validationKeyPress(key, pressedByComputer) {
    ...
    if (!ACCEPT_KEYS.includes(key))
        return false;
    if ((GAME_STATUS.round == 0.5 && key === "k") || (GAME_STATUS.round == 1.5 && key === "a"))
        return false;
    if (GAME_STATUS.round == 0 && PLAYER_TWO_KEYS.includes(key))
        return false;
    if (GAME_STATUS.round == 1 && PLAYER_ONE_KEYS.includes(key))
        return false;
    if ((GAME_STATUS.mode === "1vc" && !pressedByComputer) && PLAYER_TWO_KEYS.includes(key))
        return false;
    return true;
}
